...
def parse_fondo_detalle(html, fondo_id):
    ...
    descripcion_el = soup.select_one(
        "h1 + p"
    )

    if descripcion_el:
        descripcion = descripcion_el.get_text(
            " ",
            strip=True
        )
    ...
    cards = soup.select(
        "div.shadow-\\[5px_5px_44px_0px_rgba\\(0\\,0\\,0\\,0\\.1\\)\\]"
    )
    ...
    if info_card:
        ...
        info_text = info_card.get_text(
            " ",
            strip=True
        )

        fecha_match = re.search(
            r"Información al (\d{1,2} de \w+ de \d{4})",
            info_text,
            re.IGNORECASE
        )
        ...
        if fecha_match:
            ...
            fecha_informacion = parse_spanish_date(
                fecha_match.group(1)
            )
        ...
        if patrimonio_match:
            ...
            patrimonio = parse_money(
                patrimonio_match.group(1)
            )

        links = info_card.select(
            "a[href]"
        )
    ...
    tenencias_title = soup.find(
        "h3",
        string=lambda s: s and "Tenencias" in s
    )

    if tenencias_title:

        tenencias_card = (
            tenencias_title.find_parent("div")
        )

        rows = tenencias_card.select(
            "div.flex.mb-8"
        )
        ...
        for row in rows:
            ...
            nombre = spans[0].get_text(
                " ",
                strip=True
            )

            porcentaje_text = spans[1].get_text(
                " ",
                strip=True
            )

            porcentaje = parse_percentage(
                porcentaje_text
            )
